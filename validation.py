...
model.load_state_dict(torch.load("/home/leejianglee/2020_05_segmentation/pytorch-unet/model/model_1591703530.5789328.pth"))
...
test_files = []
for dir in os.listdir(os.path.join(TEST_PATH, "aug_image/")):
    dir = dir.split('.')
    test_files.append(dir[0])
print("the size of test_files: {}.".format(len(test_files)))

X_test = np.zeros((len(test_files), 512, 512, 3), dtype=np.uint8)
Y_test = np.zeros((len(test_files), 512, 512, 1), dtype=np.bool)
...
for n, id_ in tqdm(enumerate(test_files), total=len(test_files)):
    img_path = TEST_PATH + '/aug_image/' + id_ + '.png'
